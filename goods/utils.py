...
def q_search(query):
    ...
    result = result.annotate(headline=SearchHeadline('name', que, start_sel='<span style="background-color:yellow;">', stop_sel='</span>'),)
    result = result.annotate(bodyline=SearchHeadline('description', que, start_sel='<span style="background-color:yellow;">', stop_sel='</span>'),)
    return result
